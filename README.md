...
- 客户名称录入后自动调用客户标签识别大模型 Agent，识别行业、地区、企业属性、上市状态等客户画像标签；未配置时明确使用本地规则兜底
- Word / PPT / PDF / Excel 等材料上传入口与逐份材料描述
- 运行客户诊断时展示材料整理、大模型材料分析、关键词抽取、综合诊断进度
- 可配置客户标签识别 Agent 和材料分析大模型 Agent，将客户名称、上下文、材料描述和上传文件提交给模型服务进行结构化分析
