...
- 客户名称录入后默认调用公司大模型配置，识别行业、地区、企业属性、上市状态等客户画像标签；未配置 API Key 或接口不可用时明确使用本地规则兜底
- Word / PPT / PDF / Excel 等材料上传入口与逐份材料描述
- 运行客户诊断时展示材料整理、大模型材料分析、关键词抽取、综合诊断进度
- 平台配置默认预置大模型 Base URL 和模型名，也可配置客户标签专用 Agent 和材料分析大模型 Agent
...
客户名称输入后，若未配置“客户标签专用 Agent 接口”，前端会直接调用上述大模型，要求模型返回 JSON 企业画像标签。API Key 只保存在当前浏览器 localStorage，不写入代码仓库。

如果需要接入独立的客户标签 Agent，配置页中的“客户标签专用 Agent 接口”建议接收：
